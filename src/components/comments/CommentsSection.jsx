import { useState } from "react";
import CommentItem from "./CommentItem";
import {
  useRootComments,
  useReplies,
  useCommentMutations, // ← добавь это
} from "../../hooks/useComments";

function Replies({ commentId }) {
  const [page, setPage] = useState(1);
  const { data, isLoading } = useReplies(commentId, page, 10);
  if (isLoading)
    return <div style={{ color: "#8a8f98" }}>Загрузка ответов…</div>;
  if (!data || data.items?.length === 0) return null;

  return (
    <div
      style={{
        borderLeft: "2px solid #2a2f3a",
        marginLeft: 12,
        paddingLeft: 12,
      }}
    >
      {data.items.map((n) => (
        <CommentItem key={n.id} node={n} />
      ))}
      {data.hasMore && (
        <button onClick={() => setPage((p) => p + 1)}>Показать ещё</button>
      )}
    </div>
  );
}

export default function CommentsSection({ imdbId }) {
  const [sort, setSort] = useState("top");
  const [page, setPage] = useState(1);
  const { data, isLoading } = useRootComments(imdbId, page, 20, sort);
  const { create, update, remove, react } = useCommentMutations(imdbId); // ← теперь определено

  const reply = (parentId, text) => create.mutate({ parentId, imdbId, text });
  const edit = (id, text) => update.mutate({ id, payload: { text } });
  const del = (id) => remove.mutate(id);
  const reactFn = (id, reaction) => react.mutate({ id, reaction });

  return (
    <section>
      <div
        style={{
          display: "flex",
          justifyContent: "space-between",
          alignItems: "center",
          marginBottom: 8,
        }}
      >
        <h3 style={{ margin: 0 }}>Комментарии</h3>
        <select
          value={sort}
          onChange={(e) => {
            setSort(e.target.value);
            setPage(1);
          }}
        >
          <option value="top">Топ</option>
          <option value="new">Новые</option>
        </select>
      </div>

      <div style={{ marginBottom: 12 }}>
        <textarea
          rows={3}
          placeholder="Написать комментарий…"
          id="newComment"
          style={{ width: "100%" }}
        />
        <div
          style={{ display: "flex", justifyContent: "flex-end", marginTop: 6 }}
        >
          <button
            onClick={() => {
              const el = document.getElementById("newComment");
              const text = el.value.trim();
              if (text)
                create.mutate(
                  { imdbId, text },
                  { onSuccess: () => (el.value = "") }
                );
            }}
          >
            Отправить
          </button>
        </div>
      </div>

      {isLoading && <div style={{ color: "#8a8f98" }}>Загрузка…</div>}
      {!isLoading && data && data.items?.length === 0 && (
        <div style={{ color: "#8a8f98" }}>Комментариев пока нет</div>
      )}

      {data?.items?.map((n) => (
        <CommentItem
          key={n.id}
          node={n}
          onReply={reply}
          onEdit={edit}
          onDelete={del}
          onReact={reactFn}
        >
          <Replies commentId={n.id} />
        </CommentItem>
      ))}

      {data?.hasMore && (
        <div
          style={{ display: "flex", justifyContent: "center", marginTop: 12 }}
        >
          <button onClick={() => setPage((p) => p + 1)}>Загрузить ещё</button>
        </div>
      )}
    </section>
  );
}
